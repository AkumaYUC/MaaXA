...
#  PasteOrderColumn   param: {"column": "material_codes"}  当前单指定列拼成多行文本 → 剪贴板 → Ctrl+V
...
#PasteOrderColumn的column参数 → TransferOrder字段名
_ORDER_FIELDS = {
...
class PasteOrderColumn(CustomAction):
    ...
    def run(
        ...
        column = json.loads(argv.custom_action_param or "{}").get("column", "")  #拉取列名参数
        ...
        pyperclip.copy("\n".join(lines))  #多行用换行拼接，金蝶表格粘贴多行会自动扩展行
        _ctrl_v()
