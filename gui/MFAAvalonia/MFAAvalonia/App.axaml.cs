...
public partial class App : Application
{
    ...
    /// <summary>
    /// 启动时按实例配置的保留期清理各实例归档目录里的过期单据副本（失败静默，不拦启动）
    /// </summary>
    private void CleanupArchivedOrdersOnStartup()
    {
        try
        {
            foreach (var processor in MaaProcessorManager.Instance.Instances)
            {
                var dir = OrderArchiveHelper.ResolveArchiveDirectory(processor.InstanceConfiguration);
                var days = OrderArchiveHelper.ResolveRetentionDays(processor.InstanceConfiguration);
                OrderArchiveHelper.CleanupExpired(dir, days);
            }
        }
        catch (Exception ex)
        {
            LoggerHelper.Error($"启动清理单据归档失败: {ex.Message}", ex);
        }
    }
