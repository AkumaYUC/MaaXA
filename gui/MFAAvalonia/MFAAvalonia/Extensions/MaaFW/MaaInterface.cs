...
public partial class MaaInterface
{
    ...
    public partial class MaaInterfaceOption : ObservableObject
    {
        ...
        /// <summary>是否为 file 类型（拖拽/浏览选择文件，值仅本次会话有效）</summary>
        [JsonIgnore]
        public bool IsFile => OptionType == "file";
        ...
        {
            if (PipelineOverride == null || (!IsInput && !IsHotkey && !IsFile)) return "{}";
    ...
    public class MaaInterfaceSelectOption
    {
        ...
        /// <summary>
        /// file 类型选中的文件路径。不落盘，改存进程级字典：任务对象在「应用预设 / 重置任务列表 /
        /// 切换资源」等路径上会被克隆重建，挂在对象上会丢；存这里则 GUI 不关就一直有效，
        /// 关掉重开进程自然清空（正是「重开要重选」的预期）。键用 option 名，interface 内唯一。
        /// </summary>
        [JsonIgnore]
        public string? FileValue
        {
            get => Name != null && SessionFileValues.TryGetValue(Name, out var value) ? value : null;
            set
            {
                if (Name != null) SessionFileValues[Name] = value;
            }
        }

        private static readonly Dictionary<string, string?> SessionFileValues = new();
