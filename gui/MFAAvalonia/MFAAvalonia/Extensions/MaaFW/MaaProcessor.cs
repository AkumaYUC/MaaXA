...
// 防止 interface 加载失败时 Toast 重复显示
    ...
    //本次任务队列里用户拖入/选择的单据文件路径（file 类型 option 解析出来），
    //全部成功后由 DisplayTaskCompletionMessage 消费归档，失败/停止则留着下次 StartTask 覆盖
    private string? _pendingArchiveFile;
    ...
    public async Task StartTask(List<DragItemViewModel>? tasks, bool onlyStart = false, bool checkUpdate = false)
    {
        ...
        _pendingArchiveFile = null;  //新队列开始，清掉上一轮的归档挂起
        ...
        HashSet<string>? processedOptions = null)
    {
        ...
        foreach (var selectOption in optionsToProcess)
        {
            ...
            // 处理 file 类型（拖拽 / 浏览选中的 Excel，值只在本次会话内有效）
            else if (interfaceOption.IsFile)
            {
                var filePath = selectOption.FileValue;
                // 没拖文件就整段跳过：pipeline 里 path 留空时 agent 会报错提示，比静默注入空值好
                if (string.IsNullOrWhiteSpace(filePath)) continue;
                _pendingArchiveFile = filePath;  //记住待归档文件，全部成功后 DisplayTaskCompletionMessage 消费

                if (interfaceOption.PipelineOverride == null ||
                    interfaceOption.Inputs is not { Count: > 0 } ||
                    string.IsNullOrEmpty(interfaceOption.Inputs[0].Name))
                    continue;

                var primaryName = interfaceOption.Inputs[0].Name!;
                var fileInputValues = interfaceOption.Inputs
                    .Where(input => !string.IsNullOrEmpty(input.Name))
                    .ToDictionary(
                        input => input.Name!,
                        input => input.Name == primaryName ? filePath : input.Default ?? string.Empty);

                var fileOverride = interfaceOption.GenerateProcessedPipeline(fileInputValues);
                if (!string.IsNullOrWhiteSpace(fileOverride) && fileOverride != "{}")
                {
                    taskModels.Merge(DeserializePipelineOverrideJsonc(fileOverride));
                }
            }
            // 处理 input 类型
        ...
        bool queueCompleted = false, Action? action = null)
    {
        ...
        else
        {
            ...
            if (!onlyStart)
            {
                ...
                HandleAfterTaskOperation();
                ArchivePendingOrderFile();  //全部成功才归档：复制单据副本进归档目录并清理过期副本
            }
        }
        action?.Invoke();
        _startTime = null;
    ...
    /// <summary>
    /// 队列全部成功后归档本次拖入的单据副本（无挂起文件/归档失败都不影响收尾流程）
    /// </summary>
    private void ArchivePendingOrderFile()
    {
        var file = Interlocked.Exchange(ref _pendingArchiveFile, null);
        if (string.IsNullOrWhiteSpace(file)) return;

        var archiveDir = OrderArchiveHelper.ResolveArchiveDirectory(InstanceConfiguration);
        OrderArchiveHelper.ArchiveOrderFile(file, archiveDir);
        var retentionDays = OrderArchiveHelper.ResolveRetentionDays(InstanceConfiguration);
        OrderArchiveHelper.CleanupExpired(archiveDir, retentionDays);
    }
