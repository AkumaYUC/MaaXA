...
public class TaskOptionGenerator(TaskQueueViewModel viewModel, Action saveConfigurationAction)
{
    ...
    private void AddOption(Panel panel, MaaInterface.MaaInterfaceSelectOption option, DragItemViewModel source)
    {
        ...
        else if (interfaceOption.IsFile)
        {
            control = CreateFileDropControl(option, interfaceOption);
        }
    ...
    /// <summary>
    /// file 类型：虚线拖放区 + 点击浏览，选择 Excel 文件。选中值存 MaaInterfaceSelectOption.FileValue
    /// （不落盘的会话字段，也不调 saveConfigurationAction）——GUI 不关就一直保留，关掉重开要重新选。
    /// </summary>
    private Control CreateFileDropControl(MaaInterface.MaaInterfaceSelectOption option, MaaInterface.MaaInterfaceOption interfaceOption)
    {
        var hasOptionDescription = !string.IsNullOrWhiteSpace(GetTooltipText(interfaceOption.Description, interfaceOption.Document));
        var needsHeader = hasOptionDescription;
        ...
        var input = interfaceOption.Inputs[0];
        if (string.IsNullOrEmpty(input.Name)) return container;

        // 虚线拖放区（Border 铺虚线边框 + 提示文字 + 浏览按钮）
        ...
        dropBorder.Bind(Border.BorderBrushProperty, new DynamicResourceExtension("SukiControlBorderBrush"));

        var dropContent = new StackPanel { Spacing = 6, HorizontalAlignment = HorizontalAlignment.Stretch };
        ...
        hintText.Bind(TextBlock.TextProperty, new ResourceBinding(input.DisplayName));
        hintText.Bind(TextBlock.ForegroundProperty, new DynamicResourceExtension("SukiLowText"));

        // file 类型复用 input 的 name/label 结构：占位符替换走同一条链路
        // 套一层横向可滚动的容器：路径长了不会撑破虚线框，鼠标悬在上面滚滚轮就能看全
        var hintScroll = new ScrollViewer
        {
            HorizontalScrollBarVisibility = ScrollBarVisibility.Hidden,
            VerticalScrollBarVisibility = ScrollBarVisibility.Disabled,
            Content = hintText,
        };
        EnableWheelHorizontalScroll(hintScroll);
        dropContent.Children.Add(hintScroll);
        ...
        browseBtn.Bind(Button.ContentProperty, new ResourceBinding(input.Description));
        browseBtn.Click += async (_, _) =>
        {
            var topLevel = TopLevel.GetTopLevel(dropBorder);
            ...
            var result = await topLevel.StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
            {
                ...
                FileTypeFilter = new List<FilePickerFileType>
                {
                    new("Excel")
                    {
                        Patterns = new List<string> { "*.xlsx", "*.xlsm" },
                    },
                },
            });
            if (result.Count > 0)
            {
                ApplyDroppedFile(result[0].Path.LocalPath, dropBorder);
            }
        };
        dropContent.Children.Add(browseBtn);

        dropBorder.Child = dropContent;

        // 拖拽：只收 xlsx/xlsm，其他格式提示后忽略
        DragDrop.SetAllowDrop(dropBorder, true);
        dropBorder.AddHandler(DragDrop.DragOverEvent, (_, e) =>
        {
            e.DragEffects = DragDropEffects.Copy;
        });
        dropBorder.AddHandler(DragDrop.DropEvent, (_, e) =>
        {
...
#pragma warning restore CS0618
            ...
            {
                ToastHelper.Info($"不支持的文件类型: {ext ?? "(无后缀)"}，请拖入 .xlsx / .xlsm");
                return;
            }
            ApplyDroppedFile(path, dropBorder);
        });

        container.Children.Add(dropBorder);

        // 面板每次重建（点开始任务、切资源、删任务都会重建）时还原已选文件，用户不用重拖
        if (!string.IsNullOrWhiteSpace(option.FileValue))
        {
            ApplyDroppedFile(option.FileValue!, dropBorder);
        }
        ...
        {
            option.FileValue = path;  //存会话字段，不落盘；GUI 关掉重开才要重新选
            border.Background = new SolidColorBrush(Color.FromArgb(40, 0, 128, 255));  //已选状态的淡蓝底示意
            hintText.Text = path;  //显示完整路径让用户确认
            ToolTip.SetTip(hintScroll, path);  //悬停能看到完整路径（框太窄时不用滚）
        }
    }

    /// <summary>
    /// 给横向可滚动区域挂「悬停滚轮横向滚动」。文本类控件默认只有拿到焦点时才自动滚动，
    /// 长路径不点进去就看不全；滚轮事件标记为已处理，避免连带滚动外层页面。
    /// </summary>
    private static void EnableWheelHorizontalScroll(Control host)
    {
        host.AddHandler(InputElement.PointerWheelChangedEvent, (_, e) =>
        {
            var scrollViewer = host as ScrollViewer
                               ?? host.GetVisualDescendants().OfType<ScrollViewer>().FirstOrDefault();
            if (scrollViewer == null) return;

            var maxOffset = Math.Max(0, scrollViewer.Extent.Width - scrollViewer.Viewport.Width);
            if (maxOffset <= 0) return;  //没超出可视宽度，不用滚
    ...
    /// <summary>
    /// 归档目录选择（挂在单据拖放区下方）。值走实例配置键落盘——用户定一次就一直有效，
    /// 关掉 GUI 重开仍在，除非用户自己改。留空则用发行根下的「已处理单据」。
    /// </summary>
    private Control CreateArchiveDirectoryRow()
    {
        var instanceConfig = viewModel.Processor.InstanceConfiguration;

        var textBox = new TextBox
        {
            Text = instanceConfig.GetValue(ConfigurationKeys.OrderArchiveDirectory, string.Empty),
            Watermark = "留空用默认目录",
        };
        ToolTip.SetTip(textBox, "留空则在程序所在目录下建「已处理单据」；路径超长时鼠标悬在框上滚滚轮可横向查看");

        // 浏览按钮放在输入框外面：SukiUI 的 TextBox 模板把 InnerRightContent 和文本排在同一个 Grid 里，
        // 而模板 Padding 是固定值、不会为按钮让位，路径一长文本就横穿到按钮上
        var browseBtn = new Button
        {
            Content = "浏览",
        ...
        browseBtn.Click += async (_, _) =>
        {
            ...
            if (folders is { Count: > 0 } && folders[0].TryGetLocalPath() is { } path)
            {
                textBox.Text = path;
            }
        };
        ...
        textBox.TextChanged += (_, _) =>
            instanceConfig.SetValue(ConfigurationKeys.OrderArchiveDirectory, textBox.Text ?? string.Empty);

        // 路径长了框里放不下：悬停滚滚轮横向查看，不用点进去
        EnableWheelHorizontalScroll(textBox);
